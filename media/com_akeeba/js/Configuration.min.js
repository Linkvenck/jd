...
akeeba.Configuration.FtpTest.testConnection=function(a,b){var d=document.getElementById("var["+a+"]");akeeba.Configuration.FtpTest.buttonKey="var["+a+"]";d.setAttribute("disabled","disabled");d={host:document.getElementById("var["+b+".host]").value,port:document.getElementById("var["+b+".port]").value,user:document.getElementById("var["+b+".user]").value,pass:document.getElementById("var["+b+".pass]").value,initdir:document.getElementById("var["+b+".initial_directory]").value,usessl:document.getElementById("var["+
b+".ftps]").checked,passive:document.getElementById("var["+b+".passive_mode]").checked};akeeba.System.params.AjaxURL=akeeba.Configuration.URLs.testFtp;console.log(d);console.log(akeeba.System.params.AjaxURL);akeeba.System.doAjax(d,function(a){document.getElementById(akeeba.Configuration.FtpTest.buttonKey).removeAttribute("disabled");var c=document.getElementById("testFtpDialogBodyOk"),b=document.getElementById("testFtpDialogBodyFail"),d=document.getElementById("testFtpDialogLabel");c.style.display=
"none";b.style.display="none";!0===a?(d.textContent=Joomla.JText._("COM_AKEEBA_CONFIG_DIRECTFTP_TEST_OK"),c.textContent=Joomla.JText._("COM_AKEEBA_CONFIG_DIRECTFTP_TEST_OK"),c.style.display="block",b.style.display="none"):(d.textContent=Joomla.JText._("COM_AKEEBA_CONFIG_DIRECTFTP_TEST_FAIL"),b.textContent=a,c.style.display="none",b.style.display="block");akeeba.Modal.open({inherit:"#testFtpDialog",width:"80%"})},null,!1,15E3)};
akeeba.Configuration.SftpTest.testConnection=function(a,b){var d=document.getElementById("var["+a+"]");akeeba.Configuration.SftpTest.buttonKey="var["+a+"]";d.setAttribute("disabled","disabled");d={host:document.getElementById("var["+b+".host]").value,port:document.getElementById("var["+b+".port]").value,user:document.getElementById("var["+b+".user]").value,pass:document.getElementById("var["+b+".pass]").value,initdir:document.getElementById("var["+b+".initial_directory]").value,privkey:document.getElementById("var["+
b+".privkey]").value,pubkey:document.getElementById("var["+b+".pubkey]").value};akeeba.System.params.AjaxURL=akeeba.Configuration.URLs.testSftp;akeeba.System.doAjax(d,function(a){document.getElementById(akeeba.Configuration.SftpTest.buttonKey).removeAttribute("disabled");var b=document.getElementById("testFtpDialogBodyOk"),d=document.getElementById("testFtpDialogBodyFail"),g=document.getElementById("testFtpDialogLabel");b.style.display="none";d.style.display="none";!0===a?(g.textContent=Joomla.JText._("COM_AKEEBA_CONFIG_DIRECTSFTP_TEST_OK"),
b.textContent=Joomla.JText._("COM_AKEEBA_CONFIG_DIRECTSFTP_TEST_OK"),b.style.display="block",d.style.display="none"):(g.textContent=Joomla.JText._("COM_AKEEBA_CONFIG_DIRECTSFTP_TEST_FAIL"),d.textContent=a,b.style.display="none",d.style.display="block");akeeba.Modal.open({inherit:"#testFtpDialog",width:"80%"})},null,!1,15E3)};
...
b=akeeba.Configuration.engines[f][k.value];c={};c[b.information.title]=b.parameters;akeeba.Configuration.parseGuiData(c,h);c=h.querySelector("header");c instanceof Element&&c.insertAdjacentHTML("afterend",'<p class="akeeba-block--information">'+b.information.description+"</p>");b=[document.getElementById(g+"_config").querySelectorAll("input"),document.getElementById(g+"_config").querySelectorAll("select")];for(i=0;i<b.length;i++)if(c=b[i],c.length)for(j=0;j<c.length;j++)d=c[j],e=d.id,e=a[e],null==
e||void 0==e||("checkbox"==d.getAttribute("type")?e?d.setAttribute("checked","checked"):d.removeAttribute("checked"):d.value=e,1==i&&akeeba.System.triggerEvent(d,"change"))});a=document.createElement("button");a.className="akeeba-btn--small--dark";c=document.createElement("span");c.className="akion-wrench";a.appendChild(c);c=document.createElement("span");c.innerHTML=Joomla.JText._("COM_AKEEBA_CONFIG_UI_CONFIG");a.appendChild(c);akeeba.System.addEventListener(a,"click",function(a){akeeba.System.toggleClass(h,
"akeeba-hidden-mobile");akeeba.System.toggleClass(h,"akeeba-hidden-desktop");a.preventDefault?a.preventDefault():a.returnValue=!1});c=document.createElement("span");c.innerHTML="&nbsp;";d.appendChild(k);d.appendChild(c);d.appendChild(a);d.appendChild(h);e.appendChild(d);null!=akeeba.Configuration.engines[f][b["default"]]&&(d=akeeba.Configuration.engines[f][b["default"]],e=d.information.title,a={},a[e]=d.parameters,0!=b["protected"]&&(document.createElement("span").textContent=e,k.style.display="none"),
akeeba.Configuration.parseGuiData(a,h),b=h.querySelector("header"),b instanceof Element&&b.insertAdjacentHTML("afterend",'<p class="akeeba-block--information">'+d.information.description+"</p>"))}};
...
akeeba_postprocsftp_init_browser=function(){akeeba.Configuration.FtpBrowser.initialise("engine.postproc.sftp.initial_directory","engine.postproc.sftp")};directftp_test_connection=function(){akeeba.Configuration.FtpTest.testConnection("engine.archiver.directftp.ftp_test","engine.archiver.directftp")};postprocftp_test_connection=function(){akeeba.Configuration.FtpTest.testConnection("engine.postproc.ftp.ftp_test","engine.postproc.ftp")};
directftpcurl_test_connection=function(){akeeba.Configuration.FtpTest.testConnection("engine.archiver.directftpcurl.ftp_test","engine.archiver.directftpcurl")};postprocftpcurl_test_connection=function(){akeeba.Configuration.FtpTest.testConnection("engine.postproc.ftpcurl.ftp_test","engine.postproc.ftpcurl")};directsftp_test_connection=function(){akeeba.Configuration.SftpTest.testConnection("engine.archiver.directsftp.sftp_test","engine.archiver.directsftp")};
postprocsftp_test_connection=function(){akeeba.Configuration.SftpTest.testConnection("engine.postproc.sftp.sftp_test","engine.postproc.sftp")};directsftpcurl_test_connection=function(){akeeba.Configuration.SftpTest.testConnection("engine.archiver.directsftpcurl.sftp_test","engine.archiver.directsftpcurl")};postprocsftpcurl_test_connection=function(){akeeba.Configuration.SftpTest.testConnection("engine.postproc.sftpcurl.sftp_test","engine.postproc.sftpcurl")};
...
akconfig_googledrive_openoauth=function(){var a=akeeba.Configuration.URLs.dpeauthopen,a=-1==a.indexOf("?")?a+"?":a+"&";window.open(a+"engine=googledrive","akeeba_googledrive_window","width=1010,height=500")};akeeba_googledrive_oauth_callback=function(a){document.getElementById("var[engine.postproc.googledrive.access_token]").value=a.access_token;document.getElementById("var[engine.postproc.googledrive.refresh_token]").value=a.refresh_token;myWindow=window.open("","akeeba_googledrive_window");myWindow.close()}; //# sourceMappingURL=Configuration.min.map
